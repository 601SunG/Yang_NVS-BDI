...
def getImages(testFilename, resultFilename):
    ...
    testImage   = sitk.ReadImage(testFilename,sitk.sitkInt8)
    example = sitk.GetArrayFromImage(testImage)
    print('='*60)
    oc1 = np.count_nonzero(example == 1)
    print(oc1)
    print('this is the max value of GD')
    print(np.max(example))
    testImage = testImage[:,:,5:55]
    resultImage = sitk.ReadImage(resultFilename,sitk.sitkInt8)
    example1 = sitk.GetArrayFromImage(resultImage)
    print('='*60)
    oc2 = np.count_nonzero(example1 == 1)
    print(oc2)
    print('this is the max value of prediction')
    print(np.max(example1))
...
def getLesionDetection(testImage, resultImage):
    ...
    # recall = (number of detected WMH) / (number of true WMH)+1e-6
    recall = float(len(np.unique(lResultArray)) - 1) / ((len(np.unique(ccTestArray)) - 1)+1e-6)
    ...
    precision = float(len(np.unique(lResultArray)) - 1) / float(len(np.unique(ccResultArray)) - 1+1e-6)
